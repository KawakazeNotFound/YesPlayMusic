...
-----END PUBLIC KEY-----`;
    ...
    newLen16Rand() {
        ...
        return [Buffer.from(randByte, 'utf8'), Buffer.from(randByteReverse, 'utf8')];
    }
    ...
    apiParamsEncode(data) {
        ...
        // 第一次AES加密
        const firstCiphertext = this.aesEncryptCBC(jsonData, this.presetKey, this.iv);
        const firstCiphertextBase64 = firstCiphertext.toString('base64');
        
        console.log('[DEBUG] 第一次AES加密结果:', firstCiphertextBase64);
        
        // 第二次AES加密
        const secondCiphertext = this.aesEncryptCBC(Buffer.from(firstCiphertextBase64, 'utf8'), secondSecretKey, this.iv);
    ...
    updateGlobalCookies(setCookieHeaders) {
        ...
        setCookieHeaders.forEach(cookieStr => {
            ...
            if (name && value) {
                // 移除已存在的同名cookie
                this.globalCookies = this.globalCookies.filter(c => c.name !== name.trim());
                // 添加新cookie
    ...
    async callWeapi(api, data) {
        ...
        if (this.globalCookies.length > 0) {
            cookieString += '; ' + this.globalCookies.map(c => `${c.name}=${c.value}`).join('; ');
        }
    ...
    // 生成随机设备ID，严格按照 go-musicfox 的 GenerateSDeviceId 逻辑
    generateSDeviceId() {
        const randomNum = Math.floor(Math.random() * 1000000);
        return `unknown-${randomNum}`;
    }
    ...
    async getQRKey() {
        ...
        if (!responseData.unikey) {
            throw new Error('响应中缺少 unikey 字段: ' + JSON.stringify(responseData));
        }
            ...
            try {
                ...
                switch (status.code) {
                    case 801:
                        ...
                        break;
                    case 802:
                        console.log(`[${pollCount + 1}/${maxPoll}] 已扫码，请在APP上确认登录...`);
                        break;
                    case 803:
                        ...
                        return status;
                    case 800:
                        console.log(`[${pollCount + 1}/${maxPoll}] ❌ 二维码已过期，请重新获取`);
                        return null;
                    default:
                        console.log(`[${pollCount + 1}/${maxPoll}] ❓ 未知状态: ${status.code} - ${status.message || '无消息'}`);
                        break;
                }
...
async function testUserLikedMusic() {
    ...
    try {
        ...
        if (accountResult.code && accountResult.code === 200) {
            ...
            console.log('用户ID:', accountResult.account?.id);
            console.log('用户名:', accountResult.account?.userName);
            console.log('昵称:', accountResult.profile?.nickname);
            
            const uid = accountResult.account?.id;
            if (!uid) {
            ...
            if (playlistResult.code && playlistResult.code === 200) {
                ...
                console.log('歌单数量:', playlistResult.playlist?.length || 0);
                
                // 找到"我喜欢的音乐"歌单（通常是第一个，并且creator.userId === uid）
                const likedPlaylist = playlistResult.playlist?.find(p => 
                ...
                // 显示前几个歌单
                console.log('\n用户歌单列表（前5个）:');
                playlistResult.playlist?.slice(0, 5).forEach((playlist, index) => {
                    console.log(`${index + 1}. ${playlist.name} (${playlist.trackCount}首歌)`);
                });
            } else {
            ...
            if (likeListResult.code && likeListResult.code === 200) {
                ...
                console.log('喜欢的歌曲数量:', likeListResult.ids?.length || 0);
                ...
                if (likeListResult.ids && likeListResult.ids.length > 0) {
                    ...
                    fs.writeFileSync(resultPath, JSON.stringify({
                        ...
                        userInfo: {
                            ...
                            nickname: accountResult.profile?.nickname
                        },
                ...
                console.log('❌ 获取喜欢的音乐列表失败:', likeListResult.message || '未知错误');
            }
            
        } else {
            console.log('❌ 获取用户账户信息失败:', accountResult.message || '可能需要重新登录');
        }
